...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center bg-light border-0">
        <div class="row">
            ...
            <div class="col-6">
                <select style="width: fit-content;" class="form form-control" onchange="location = this.value;">
                    @if(isset($my_class) && $my_class->id > 0)
                    <option value="{{ route('students.list', $my_class->id) }}" selected>{{ $my_class->name }}</option>
                    @else
                    <option value="{{ route('students.list', 0) }}" selected>-- Select Class --</option>
                    @endif
                    @foreach(App\Models\MyClass::orderBy('name')->get() as $c)
                    <option value="{{ route('students.list', $c->id) }}">{{ $c->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
                ...
                <table class="table datatable-button-html5-columns">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php
                                // $filename = $s->user->photo;
                                // $file_loc = Qs::profile_picture($filename);
                                // echo $file_loc;
                                // echo '<br/>';
                                // // echo $filename;
                                ?><img class="rounded-circle" style="height: 40px; width: 40px;" src="{{Qs::profile_picture($s->user->photo) }}" alt="photo"></td>
                            <td>{{ $s->user->name }}</td>
                            <td>{{ $s->adm_no }}</td>
                                ...
                                <div class="list-icons">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-left">
                                            <a href="{{ route('students.show', Qs::hash($s->id)) }}" class="dropdown-item"><i class="icon-eye text-n"></i> View Profile</a>
                                            @if(Qs::userIsTeamSA())
